export const createNetworkInterface = (chainId, customNetworks) => {
    const knownChainId = knownNetworks.map((network) => network.chainId);
    if ([1, 3, 4, 5, 42].includes(chainId)) {
        const defaultNetwork = defaultNetworks.find((network) => network.chainId === chainId);
        if (defaultNetwork) {
        ...
        const knownNetwork = knownNetworks.find((network) => network.chainId === chainId);
        if (knownNetwork) {
            return knownNetwork;
        }
        else {
        ...
        if (customNetwork) {
            ...
            console.warn('No network match, make sure the network is known by "react-ether" or added in the config');
            return networkNull;
        }
    }
};
export const extractEndpoints = (chainId, customNetworks) => {
    const knownChainId = knownNetworks.map((network) => network.chainId);
    if ([1, 3, 4, 5, 42].includes(chainId)) {
        return [];
    }
